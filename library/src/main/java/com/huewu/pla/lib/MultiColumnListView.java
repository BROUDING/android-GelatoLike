...
import com.brouding.gelatolike.R;
import com.huewu.pla.lib.internal.ParcelableSparseIntArray;
...
public class MultiColumnListView extends PLA_ListView {
	...
	public int mdefaultColumnWidth = 0;
	public int mdefaultColumnHeight = 0;
...
//		init(null, columnNumber);					// react-native의 createViewInstance에서 param을 받을 수 없기에 init을 동적으로 가능하게 하였다. (private >> public)
	...
	public void init(AttributeSet attrs, int columnNumber) {
		getWindowVisibleDisplayFrame(mFrameRect);

		if( attrs == null ){
			mColumnNumber = columnNumber;	//DEFAULT_COLUMN_NUMBER; 	//default column number is 2.
		}else{
...
//		Log.e("@@# getMeasuredWidth = ", "" +getMeasuredWidth());
	...
	private Column getNextColumn(boolean flow, int position) {
		...
		//adjust position (excoadude headers...)
	...
	public interface OnCustomScrollListener {
		void onFirstLineForScrollTopButton();
		void onNotFirstLineForScrollTopButton();
		/**
...
//			Log.e("@@# complete? = ", "" +loadingMoreComplete);
			if (//scrollState == OnScrollListener.SCROLL_STATE_IDLE &&
				...
				customScrollListener.onLoadMore();
			}
		}
		...
		public void onScroll(PLA_AbsListView view, int firstVisibleItem,
			...
			if( isAutoScrolling ) {
				int topX = mColumns[0].getTop();
				if( topX!=0 ) {
					smoothScrollToPosition( topX );
				}
			}

			if( firstVisibleItem == 0 ) {
				isAutoScrolling = false;
				customScrollListener.onFirstLineForScrollTopButton();
			} else if( firstVisibleItem > 4 ) {
				customScrollListener.onNotFirstLineForScrollTopButton();
			}
		}
	};
	OnCustomScrollListener customScrollListener;

	public void setOnCustomScrollListener(OnCustomScrollListener listener) {
		if (listener != null) {
			this.customScrollListener = listener;
